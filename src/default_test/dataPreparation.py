...
def dataset_to_csv_based_on_sensor_events_time_Ordered(file_address_to_read, file_address_to_save):
    '''
...
def dataset_create_different_feature_vectores(raw_data_address, add_str_to_path, header):
    ...
    each_row_one_feature = r"E:\pgmpy\{}\sensor_data_each_row_one_features_is_one_on_and_off+time_ordered.csv".format(add_str_to_path)
    ...
    #for i in [180,200,240,300,400,500,600,700,800,900,1000]:#range(1,16)[1600,1800,2000,2500,3000,3500,4000,4500,5000]
        ...
        casas7_create_bag_of_sensor_events_based_on_activity_and_delta(deltaInMinutes=i , 
                                                                       ...
                                                                       address_to_read=each_row_one_feature, 
                                                                       address_for_save= bag_activity_and_delta.format(path = add_str_to_path, delta = i), 
                                                                       isSave = True,
                                                                       header = header)       
        ...
        create_sequence_of_sensor_events_based_on_activity_and_delta(deltaInMinutes = i, 
                                                                     address_to_read = each_row_one_feature, 
                                                                     has_header = False, 
                                                                     address_for_save = seq_activity_and_delta.format(path = add_str_to_path, delta = i), 
                                                                     isSave = True,
																	 header = header)
...
def casas7_create_bag_of_sensor_events_no_overlap(deltaInMinutes ,number_of_entire_rows, address_to_read, address_for_save, isSave, header):
    '''
    1. The order of  features: sensor events (for each sensor on and off), 
                               Person,
                               Date,
    ...
    #print("was opened")
    all_features = None#np.zeros((number_of_entire_rows, 126), dtype= object )#np.str)1003 +1
    ...
    for line in f:
        ...
        if first:
            number_of_columns = len(cells) # +1 is for datetime column
        ...
        cells[number_of_columns-1] = cells[number_of_columns-1].split('\n')[0]# the 0th element is the time, the next is empty
        #print(cells)
        
        converted_cells = [int(i) for i in cells[0:number_of_columns-3]]  
        
        if first == True:
            print(len(converted_cells)) 
        
        converted_cells.append(cells[number_of_columns-2]) #  is date
        converted_cells.append(cells[number_of_columns-1]) #  is time
        #print("counter:{}".format(counter))
        converted_cells.append(convert_string_to_datetime(cells[number_of_columns-2], cells[number_of_columns-1])) # 126 is datetime object that is used for comparision but is removed before saving file
    ...
    for each_person in range(number_of_residents):
        ...
        person_bag[each_person] = np.ndarray(shape = (person_data_number_of_rows , number_of_columns-4), dtype = int )
        #print(person_bag[each_person].shape)
        for i in range(number_of_columns-4):
        ...
        for offset in range(1, len(person_data[each_person]), 1): # range(start, end, step) sharte end ine k bozorgtar bashe pas bayad yeki az akhari kamtar begiram
            ...
            # compare delta time in minutes
            if timedelta_in_minutes <= deltaInMinutes:
                for i in range(number_of_columns-4):# 120 is num of features
        ...
        person_bag[each_person] = person_bag[each_person][0:new_counter +1, :]#np.delete(person_bag[each_person] , range(new_counter + 1 , person_data_number_of_rows + 1 ) , axis = 0)    
    ...
    if isSave == True:
        ...
        np.savetxt(address_for_save, person_bag[0] , delimiter=',' , fmt='%s' , header = header.replace(',Work' , '' ))
    
    
    return person_bag[0]


def casas7_create_bag_of_sensor_events_based_on_activity(number_of_entire_rows, address_to_read, address_for_save, isSave, header):
    '''
    1. The order of  features: sensor events (for each sensor on and off), 
                               Person,
                               Date,
    ...
    for line in f:
        ...
        if first:
            number_of_columns = len(cells) -2 # +1 is for datetime column
            print("number_of_columns:",number_of_columns)
            all_features = np.zeros((number_of_entire_rows, number_of_columns), dtype= object )#np.str)1003 +1


        converted_cells = [int(i) for i in cells[0:number_of_columns]]  
        ...
        if first == True:
            first  = False
            print(len(converted_cells))
            print(converted_cells[number_of_columns-1])
    ...
    for each_person in range(number_of_residents):
        ...
        person_data_number_of_rows , _ = person_data[each_person].shape
        # create a ndarray with size of all data of the person 
        person_bag[each_person] = np.ndarray(shape = (person_data_number_of_rows , number_of_columns), dtype = int )
        #print(person_bag[each_person].shape)
        ...
        for offset in range(1, len(person_data[each_person]), 1): # range(start, end, step) sharte end ine k bozorgtar bashe pas bayad yeki az akhari kamtar begiram
            ...
            if person_data[each_person][offset][-1] == last_activity:
                for i in range(number_of_columns-2):# 120 is num of features
                    person_bag[each_person][new_counter][i] += person_data[each_person][offset][i]
    ...
    if isSave == True:
        ...
        np.savetxt(address_for_save, person_bag[0] , delimiter=',' , fmt='%s' , header = header)
     
    return person_bag[0]


def casas7_create_bag_of_sensor_events_based_on_activity_and_delta(deltaInMinutes, number_of_entire_rows, address_to_read, address_for_save, isSave, header):
    '''
    1. The order of  features: sensor events (for each sensor on and off), 
                               Person,
                               Date,
    ...
    for line in f:
        ...
        cells = line.split(',')
        if first:
            number_of_columns = len(cells) + 1 # +1 is for datetime column
            print("number_of_columns:",number_of_columns)
            all_features = np.zeros((number_of_entire_rows, number_of_columns), dtype= object )#np.str)1003 +1

        cells[number_of_columns-2] = cells[number_of_columns-2].split('\n')[0]# the 0th element is the time, the next is empty
        #print(cells)
        ...
        converted_cells.append(cells[number_of_columns-3]) #  is date
        converted_cells.append(cells[number_of_columns-2]) #  is time
        #print("counter:{}".format(counter))
        converted_cells.append(convert_string_to_datetime(cells[number_of_columns-3], cells[number_of_columns-2])) # 126 is datetime object that is used for comparision but is removed before saving file
    ...
    for each_person in range(number_of_residents):
        ...
        person_data_number_of_rows , _ = person_data[each_person].shape
        # create a ndarray with size of all data of the person 
        person_bag[each_person] = np.ndarray(shape = (person_data_number_of_rows , number_of_columns-3), dtype = int )
        #print(person_bag[each_person].shape)
        for i in range(number_of_columns-5):
            person_bag[each_person][0][i] = person_data[each_person][0][i]
        #print(person_bag[each_person].shape)
        ...
        for offset in range(1, len(person_data[each_person]), 1): # range(start, end, step) sharte end ine k bozorgtar bashe pas bayad yeki az akhari kamtar begiram
            ...
            if person_data[each_person][offset][-4] == last_activity:
                ...
                # compare delta time in minutes
                if timedelta_in_minutes <= deltaInMinutes:
                    for i in range(number_of_columns-5):# 120 is num of features
                        person_bag[each_person][new_counter][i] += person_data[each_person][offset][i]
...
def create_sequence_of_sensor_events_based_on_activity(address_to_read,has_header, address_for_save, isSave, header):
    '''
    we imagine in each row of dataset, just one sensor event is active and the dataset is ordered base on time 
    1. The order of  features: sensor events (for each sensor on and off), 
                               Person,
...
def create_sequence_of_sensor_events_based_on_delta_no_overlap(deltaInMinutes, address_to_read,has_header, address_for_save, isSave, header):
    '''
    we imagine in each row of dataset, just one sensor event is active and the dataset is ordered base on time 
    1. The order of  features: sensor events (for each sensor on and off), 
                               Person,
...
def create_dataset_each_row_one_feature_on_plus_hour_of_day(file_address = ' ', address_to_save = ' ' , add_str_to_path = ' '):
    
    if file_address == ' ':
        file_address = r'E:\pgmpy\{path}\sensor_data_each_row_one_features_is_one_on_and_off+time_ordered.csv'
    
    if address_to_save == ' ':
        address_to_save = r'E:\pgmpy\{path}\sensor_data_each_row_one_features_is_one_on_and_off+hour_of_day.csv'
    
    file_address = file_address.format(path = add_str_to_path) 
    address_to_save = address_to_save.format(path = add_str_to_path)
...
def create_sequence_of_events_based_on_different_number_of_events(add_str_to_path):
   
    address_to_read = r'E:\pgmpy\{path}\sensor_data_each_row_one_features_is_one_on_and_off+time_ordered.csv'
    address_to_read = address_to_read.format(path = add_str_to_path)
    address_for_save = r'E:\pgmpy\{path}\Seq of sensor events_based_on_number_of_events\number_of_events={ne}.csv'
    for events in range(3,10):
        create_sequence_of_sensor_events_based_on_number_of_events(number_of_events = events, 
                                                                   ...
                                                                   has_header = False, 
                                                                   address_for_save = address_for_save.format(path = add_str_to_path ,ne = events), 
                                                                   isSave = True)
...
def create_features_based_on_iccke_idea(add_str_to_path):  

    activity_bag = r"E:\pgmpy\{path}\Bag of sensor events_based on activities\based_on_activities.csv".format(path = add_str_to_path)
    activity_bag_iccke = r"E:\pgmpy\{path}\Bag of sensor events_based on activities\based_on_activities_iccke_approach.csv".format(path = add_str_to_path)
...
def create_previous_works_feature_vectores(add_str_to_path):

    #create_features_based_on_iccke_idea(add_str_to_path)
    #create_dataset_each_row_one_feature_on_plus_hour_of_day(' ', ' ' , add_str_to_path)
    
    create_sequence_of_events_based_on_different_number_of_events(add_str_to_path)
