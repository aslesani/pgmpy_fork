...
from dataPreparation import file_header_Twor2009
...
def edit_header_of_files():
    '''
    it removes the first line of file(previous header) and add the new header line
    '''
    base_address = r"E:\pgmpy\Twor2009"
    folders = ["Bag of sensor events_based on activities",
               "Bag of sensor events_based_on_activity_and_no_overlap_delta",
               "Bag of sensor events_no overlap_based on different deltas"]

    for mypath in [(base_address + "\\" +f) for f in folders]: 
        list_of_files = [join(mypath, f) for f in listdir(mypath) if isfile(join(mypath, f))]
        for f in list_of_files:
            
            remove_first_line_from_file(f)
            line_prepender(filename = f, line = "# " + file_header_Twor2009)
        
    
        
if __name__ == "__main__":
    #previous_line_editor()
    edit_header_of_files()
